...
import { Box, Button, Card, Flex, Heading, Spinner, Stack, Text, useToast } from '@sanity/ui'
...
const OrderSyncTool = () => {
  ...
  useEffect(() => {
    const fetchLastSyncInfo = async () => {
      try {
        ...
        if (syncState?.lastSyncTime) {
          const timeAgo = formatDistanceToNow(new Date(syncState.lastSyncTime), { addSuffix: true })
    ...
    const intervalId = setInterval(() => {
      if (lastSyncInfo?.lastSyncTime) {
        const timeAgo = formatDistanceToNow(new Date(lastSyncInfo.lastSyncTime), { addSuffix: true })
        setLastSyncTimeAgo(timeAgo)
      }
    }, 60000)
  ...
  return (
    <Flex padding={4} direction="column" align="flex-start">
      <Box marginBottom={5}>
        <Heading size={2}>Order Sync Tool</Heading>
        <Text margintop={3}>
          This tool synchronizes orders from your database to Sanity. Use this after making changes to orders
          outside of Sanity or if you notice that the orders are out of sync.
        </Text>
        {lastSyncTimeAgo && (
          <Text margintop={2} size={1} muted>
            {`Last synced: ${lastSyncTimeAgo} • Status: `}
            <span style={{ color: lastSyncInfo?.syncStatus === 'success' ? 'green' : 'red' }}>
      ...
      <Card padding={4} radius={2} shadow={1} tone="primary">
        <Stack space={4}>
          <Box>
            ...
            <Text margintop={2}>
              Click the button below to sync all orders from your database to Sanity.
              This process may take a few moments depending on the number of orders.
            </Text>
          </Box>
          ...
          {syncResult && (
            <Card padding={3} radius={2} tone={syncResult.status === 'success' ? 'positive' : 'critical'}>
              <Stack space={3}>
                <Heading size={1}>Sync {syncResult.status === 'success' ? 'Completed' : 'Failed'}</Heading>
                ...
                {syncResult.status === 'success' ? (
                  ...
                  <Text>{syncResult.message || 'An unknown error occurred'}</Text>
                )}
              </Stack>
            </Card>
          )}
