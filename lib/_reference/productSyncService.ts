...
export async function syncProductToDB(sanityProduct: any) {
  try {
    ...
    let existingProduct = await prisma.product.findFirst({
      where: {
        OR: [{ id: sanityProduct._id }, { slug: productData.slug }],
      },
      include: {
        variants: true,
      },
    ...
    if (sanityProduct.variants && Array.isArray(sanityProduct.variants)) {
      ...
      if (variantTypes.includes("size")) {
        ...
        const colors = variantTypes.includes("color")
          ? variantOptions["color"]
          : ["default"];

        for (const size of sizes) {
          for (const color of colors) {
            const sku = `${sanityProduct._id}-${size}-${color}`
              .replace(/\s+/g, "-")
              .toLowerCase();
        ...
        for (const existingVariant of existingProduct.variants) {
          ...
          const colorStillExists = colors.includes(
            existingVariant.color || "default",
          );
    ...
    console.error(
      `Error syncing product ${sanityProduct._id} to database:`,
      error,
    );
    throw error;
  }
    ...
    for (const product of sanityProducts) {
      try {
        ...
        const existingProduct = await prisma.product.findFirst({
          where: {
            OR: [{ id: product._id }, { slug: product.slug?.current || "" }],
          },
        });
...
export async function handleSanityProductUpdate(product: any) {
  try {
    ...
    console.error(
      `Error handling Sanity product update for ${product._id}:`,
      error,
    );
    throw error;
  }
...
export async function handleSanityProductDelete(productId: string) {
  try {
    ...
    console.error(
      `Error handling Sanity product deletion for ${productId}:`,
      error,
    );
    throw error;
  }
